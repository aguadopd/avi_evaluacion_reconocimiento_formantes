...
if muestras > length(audio)
    izq = 1;
    der = length(audio);
    warning('LPC:EXTRAER: Se están requiriendo más muestras de las que existen. Se usarán todas las disponibles');
    audio = audio(izq:der);
    centro = floor(length(audio+1)/2);
else
...
lpc_orden = obj.parametros('orden');
...
formantes_estimados = [F1_lpc, F2_lpc];
